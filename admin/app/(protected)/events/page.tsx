...
export default async function EventsPage({
  ...
  const { rows, total } = await adminEventsRepo.listEvents({ search, sort, page, pageSize: PAGE_SIZE });
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <EventsTable rows={rows} total={total} page={page} pageSize={PAGE_SIZE} search={search} sort={sort ?? ""} />
    </div>
  );
}
